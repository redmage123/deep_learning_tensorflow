...
with tf.name_scope('linear_nodes'):
...
# Let's now calculate the cost of the prediction algorithm using least mean squares
    ...
    tf.summary.scalar('training cost',training_cost)
    tf.summary.scalar('unexplained cost',unexplained_cost)
...
with tf.Session() as tf_session:
    ...
    writer = tf.summary.FileWriter('/tmp/linear_demo/1')
...
# We'll run the data through for 1000 times (The value of training_epochs). 
    ...
    for epoch in range(training_epochs):
        ...
        # For every five cycles, write our merged summaries out to the disk
        if epoch % 5 == 0:
            s = tf_session.run(merged_summary,feed_dict = {X: train_X,Y: train_Y})
