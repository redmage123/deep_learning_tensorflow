...
# Here we describe the layers for our CNN. 
# First layer:  Describes the patch size of 5 X 5 with one input channel and 

conv_layers = {'conv1_layer':tf.Variable(tf.random_normal([5,5,1,32])),
               'conv2_layer':tf.Variable(tf.random_normal([5,5,32,64])),
               'fc_layer':tf.Variable(tf.random_normal([7 * 7 * 64,1024])),
               'output_layer':tf.Variable(tf.random_normal([1024,n_classes])),
